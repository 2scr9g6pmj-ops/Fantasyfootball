...
def home():
    ...
    <div id='status' class='status'>Loading synced leagues…</div>
    <section class='importer'><h2>Import weekly projections</h2><p>Fetch ESPN's complete weekly player feed automatically, or upload a CSV for the selected league and week. CSV columns: <code>player,team,position,projection</code> or <code>player_id,projection</code>.</p><div class='toolbar'><button id='espnSyncButton' onclick='syncEspn()'>Fetch all ESPN players</button><select id='importSource' aria-label='Projection source'><option value='espn'>ESPN</option><option value='sleeper'>Sleeper</option><option value='manual'>Other/manual</option></select><input id='projectionFile' aria-label='Projection CSV' type='file' accept='.csv,text/csv'><button id='importButton' onclick='importProjections()'>Upload CSV</button></div><div id='importStatus' class='status'></div></section>
    <div class='cards' id='cards'></div><div id='changes' class='empty' hidden></div><div id='empty' class='empty' hidden></div><h2 id='lineupHeading' hidden>Recommended lineup</h2><table id='lineupTable' hidden><thead><tr><th>Slot</th><th>Player</th><th>Team</th><th>Sleeper Projection</th><th>Aggregated Projection</th><th>Sources</th><th>Start Score</th><th>Health</th><th>Why</th></tr></thead><tbody id='lineup'></tbody></table><h2 id='rosterHeading' hidden>Your players</h2><table id='rosterTable' hidden><thead><tr><th>Recommendation</th><th>Player</th><th>Pos</th><th>Team</th><th>Sleeper Projection</th><th>Aggregated Projection</th><th>Sources</th><th>Health</th><th>Comment</th></tr></thead><tbody id='rosterPlayers'></tbody></table><p><a href='/draft'>Draft Room</a> · <a href='/docs'>API documentation</a></p>
    <script>
    const el=id=>document.getElementById(id);
    function showStatus(message,kind=''){el('status').textContent=message;el('status').className='status '+kind}
    async function jsonRequest(url,options){const response=await fetch(url,options),raw=await response.text();let payload;try{payload=raw?JSON.parse(raw):{}}catch{const isHtml=raw.trimStart().startsWith('<');payload={detail:isHtml?'The service took too long to respond. Please try again.':raw.slice(0,300)}}if(!response.ok)throw new Error(payload.detail||`Request failed (${response.status})`);return payload}
    function parseCsv(text){const rows=[];let row=[],value='',quoted=false;for(let i=0;i<text.length;i++){const c=text[i],next=text[i+1];if(c==='"'&&quoted&&next==='"'){value+='"';i++}else if(c==='"'){quoted=!quoted}else if(c===','&&!quoted){row.push(value.trim());value=''}else if((c==='\\n'||c==='\\r')&&!quoted){if(c==='\\r'&&next==='\\n')i++;row.push(value.trim());if(row.some(Boolean))rows.push(row);row=[];value=''}else value+=c}row.push(value.trim());if(row.some(Boolean))rows.push(row);if(rows.length<2)return[];const headers=rows.shift().map(x=>x.trim().toLowerCase());return rows.map(values=>Object.fromEntries(headers.map((header,index)=>[header,values[index]??''])))}
    async function importProjections(){const file=el('projectionFile').files[0],league=el('league').value,week=el('week').value,source=el('importSource').value;if(!league){el('importStatus').textContent='Select a league first.';return}if(!file){el('importStatus').textContent='Choose a CSV file first.';return}const button=el('importButton');button.disabled=true;el('importStatus').textContent='Uploading projections…';try{const rows=parseCsv(await file.text());if(!rows.length)throw new Error('The CSV has no data rows.');const result=await jsonRequest(`/api/leagues/${encodeURIComponent(league)}/projections/${week}/csv?source=${encodeURIComponent(source)}`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(rows)});const unmatched=result.unmatched.length?` Unmatched: ${result.unmatched.join(', ')}.`:'';el('importStatus').textContent=`Imported ${result.imported} ${source.toUpperCase()} projections.${unmatched}`;el('importStatus').className='status '+(result.imported?'good':'error');await load()}catch(error){el('importStatus').textContent=error.message;el('importStatus').className='status error'}finally{button.disabled=false}}
    async function syncEspn(){const league=el('league').value,week=el('week').value,button=el('espnSyncButton');if(!league){el('importStatus').textContent='Select a league first.';return}button.disabled=true;el('importStatus').textContent='Fetching and matching the complete ESPN player feed…';try{const result=await jsonRequest(`/api/leagues/${encodeURIComponent(league)}/projections/${week}/espn/sync`,{method:'POST'});el('importStatus').textContent=`Matched ${result.imported} of ${result.source_players} ESPN projections using player name, team, and position. ${result.unmatched_count} could not be safely matched.`;el('importStatus').className='status good';await load()}catch(error){el('importStatus').textContent=error.message;el('importStatus').className='status error'}finally{button.disabled=false}}
    ...
    async function load(){if(!el('league').value)return;showStatus('Loading projections and calculating the best lineup…');try{const url=`/api/leagues/${encodeURIComponent(el('league').value)}/recommendations/${el('week').value}?profile=${encodeURIComponent(el('profile').value)}`;const result=await jsonRequest(url);el('cards').innerHTML=`<div class=card>Current<br><b>${result.current_projected_points}</b></div><div class=card>Optimized<br><b class=good>${result.optimized_projected_points}</b></div><div class=card>Changes<br><b>${result.changes.length}</b></div><div class=card>Projection Sources<br><b>${result.projection_source}</b></div>`;el('changes').replaceChildren();if(result.changes.length){const title=document.createElement('b');title.textContent='Recommended changes';el('changes').append(title);for(const change of result.changes){const item=document.createElement('p');item.textContent=change.reason;el('changes').append(item)}el('changes').hidden=false}else{el('changes').hidden=true}el('lineup').replaceChildren();for(const player of result.recommended_lineup){const row=document.createElement('tr');const sources=Object.keys(player.projection_sources||{}).join(', ')||'none';for(const value of [player.slot,player.name,player.team||'-',player.sleeper_projection??'-',player.aggregate_projection,sources,player.start_score,player.injury||'Healthy',player.explanation]){const cell=document.createElement('td');cell.textContent=value;row.append(cell)}el('lineup').append(row)}el('rosterPlayers').replaceChildren();for(const player of result.roster_players||[]){const row=document.createElement('tr');const sources=Object.keys(player.projection_sources||{}).join(', ')||'none';const recommendation=player.recommended_start?'START':'BENCH';for(const value of [recommendation,player.name,player.position||'-',player.team||'-',player.sleeper_projection??'-',player.aggregate_projection,sources,player.injury||'Healthy',player.comment]){const cell=document.createElement('td');cell.textContent=value;row.append(cell)}el('rosterPlayers').append(row)}el('lineupTable').hidden=false;el('lineupHeading').hidden=false;el('rosterTable').hidden=false;el('rosterHeading').hidden=false;showStatus(`Week ${el('week').value} ${result.profile} recommendations are ready using ${result.projection_source} projections.`,'good')}catch(error){el('lineupTable').hidden=true;el('lineupHeading').hidden=true;el('rosterTable').hidden=true;el('rosterHeading').hidden=true;showStatus(error.message,'error')}}
