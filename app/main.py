...
def home():
    ...
    async function jsonRequest(url,options){const response=await fetch(url,options),raw=await response.text();let payload;try{payload=raw?JSON.parse(raw):{}}catch{payload={detail:raw}}if(!response.ok)throw new Error(payload.detail||`Request failed (${response.status})`);return payload}
