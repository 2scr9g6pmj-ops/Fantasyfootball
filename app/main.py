...
def home():
    ...
    async function load(){if(!el('league').value)return;showStatus('Loading projections and calculating the best lineup…');try{const url=`/api/leagues/${encodeURIComponent(el('league').value)}/recommendations/${el('week').value}?profile=${encodeURIComponent(el('profile').value)}`;const result=await jsonRequest(url);el('cards').innerHTML=`<div class=card>Current<br><b>${result.current_projected_points}</b></div><div class=card>Optimized<br><b class=good>${result.optimized_projected_points}</b></div><div class=card>Changes<br><b>${result.changes.length}</b></div><div class=card>Projections<br><b>${result.projection_source}</b></div>`;el('lineup').replaceChildren();for(const player of result.recommended_lineup){const row=document.createElement('tr');for(const value of [player.slot,player.name,player.team||'-',player.projection,player.start_score,player.injury||'Healthy']){const cell=document.createElement('td');cell.textContent=value;row.append(cell)}el('lineup').append(row)}el('lineupTable').hidden=false;showStatus(`Week ${el('week').value} recommendations are ready using ${result.projection_source} projections.`,'good')}catch(error){el('lineupTable').hidden=true;showStatus(error.message,'error')}}
